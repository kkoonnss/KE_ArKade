...
while i < len(lines):
    ...
    if l.startswith("func _create_game_card("):
        new_content.append("""func _create_game_card(cart: Dictionary, parent_grid: Container, display_index: int = -1):
	var cart_id = cart.id
...
\n""")
        ...
        while i < len(lines) and not lines[i].startswith("func _create_level_card("):
            i += 1
        continue
    elif l.startswith("func display_games("):
        new_content.append("""func display_games():
	clear_main_panel()
	current_tab = "Games"
...
\n""")
        ...
        while i < len(lines) and not lines[i].startswith("func _launch_game("):
            i += 1
        continue
    elif l.startswith("\tlist.sort_custom(func(a, b):"):
        if i + 1 < len(lines) and "order_favorites_first" in lines[i+1]:
            new_content.append("""	list.sort_custom(func(a, b):
