...
func _input(event):
	...
	if event is InputEventKey and event.pressed and not event.echo:
        if event.keycode == KEY_TAB:
            ...
            _set_overlay_mode("start")
            return
        elif game_id == "snake" and overlay_mode == "":
...
func _draw_snake():
    ...
    for cell in snake:
        centers.append(_cell_center(cell))
    if centers.size() > 1:
        draw_polyline(centers, Color(0.0, 0.9, 1.0, 0.22), cell_px * 0.62)
        draw_polyline(centers, Color(0.0, 0.9, 0.46, 0.62), cell_px * 0.38)
    for i in range(snake.size() - 1, 0, -1):
        var taper = lerp(0.22, 0.34, float(i) / max(1.0, float(snake.size() - 1)))
        _glow_circle(_cell_center(snake[i]), cell_px * taper, C_CYAN)
    var head = _cell_center(snake[0])
    var dir = Vector2(snake_dir)
    if snake.size() > 1:
        dir = Vector2(snake[0] - snake[1])
    if dir == Vector2.ZERO:
        dir = Vector2.RIGHT
    dir = dir.normalized()
    var side = Vector2(-dir.y, dir.x)
    _glow_circle(head, cell_px * 0.44, C_GREEN)
    draw_circle(head + dir * cell_px * 0.16 + side * cell_px * 0.13, max(2.5, cell_px * 0.075), Color.WHITE)
    draw_circle(head + dir * cell_px * 0.16 - side * cell_px * 0.13, max(2.5, cell_px * 0.075), Color.WHITE)
    draw_circle(head + dir * cell_px * 0.18 + side * cell_px * 0.13, max(1.0, cell_px * 0.03), Color.BLACK)
    draw_circle(head + dir * cell_px * 0.18 - side * cell_px * 0.13, max(1.0, cell_px * 0.03), Color.BLACK)
    _glow_line(head + dir * cell_px * 0.34, head + dir * cell_px * 0.52, C_MAGENTA, 1.2)
