...
func _tick_tank(delta):
    ...
    if Input.is_key_pressed(KEY_UP) or Input.is_key_pressed(KEY_W):
        player["pos"] = _tank_step(player["pos"], dir * 150 * delta)
    if Input.is_key_pressed(KEY_DOWN) or Input.is_key_pressed(KEY_S):
        player["pos"] = _tank_step(player["pos"], -dir * 100 * delta)
    player["cool"] = max(0.0, player["cool"] - delta)
    if _action() and player["cool"] <= 0:
    ...
    for e in enemies:
        ...
        e["pos"] = _tank_step(e["pos"], Vector2(cos(e["angle"]), sin(e["angle"])) * 55 * delta)
        e["cool"] -= delta
        if e["cool"] <= 0:
...
func _tick_bullets(delta):
    for i in range(bullets.size() - 1, -1, -1):
        ...
        bullets[i]["ttl"] -= delta
        if game_id == "battlezone" or game_id == "smash_tv":
            var hit_enemy = false
            for ei in range(enemies.size() - 1, -1, -1):
                var e = enemies[ei]
                var radius = 28 if e.get("kind", "") == "spawner" else 24
                if bullets[i]["pos"].distance_to(e["pos"]) < radius:
                    e["hp"] = e.get("hp", 1) - 1
                    hit_enemy = true
                    if e["hp"] <= 0:
                        _burst(e["pos"], C_MAGENTA, 18)
                        enemies.remove_at(ei)
                        score += 100 if e.get("kind", "") == "spawner" else 50
                        _emit_score()
                    break
            if hit_enemy:
                bullets.remove_at(i)
...
func _tank_step(pos: Vector2, delta_pos: Vector2) -> Vector2:
    var full = _clamp(pos + delta_pos)
    if _cell_walkable(_pos_to_cell(full)):
        return full
    var x_only = _clamp(pos + Vector2(delta_pos.x, 0))
    if _cell_walkable(_pos_to_cell(x_only)):
        return x_only
    var y_only = _clamp(pos + Vector2(0, delta_pos.y))
    if _cell_walkable(_pos_to_cell(y_only)):
        return y_only
    return _clamp(pos)
