...
func _tick_centipede(delta):
    ...
    for i in range(centipede.size()):
        var seg = centipede[i]
        if i == 0:
            var next = seg["pos"] + seg["dir"] * (105.0 + wave * 18.0) * delta
    ...
    if centipede.is_empty():
        wave += 1
        _setup_centipede()
    elif wave >= 3 and centipede.size() < min(16, 4 + wave * 2) and randf() < delta * (0.12 + wave * 0.03):
        centipede.append({"pos": _safe_position(Vector2(randf_range(map_w * 0.12, map_w * 0.88), map_h * 0.1)), "dir": Vector2([-1, 1][randi() % 2], 0), "alive": true})
