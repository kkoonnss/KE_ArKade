...
var paddle_x = 0.0
var paddle_w = 140.0
var breakout_power = {"wide": 0.0, "slow": 0.0, "laser": 0.0, "shield": 0.0}
...
func _setup_breakout():
    paddle_x = map_w * 0.5
    paddle_w = 140.0
    breakout_power = {"wide": 0.0, "slow": 0.0, "laser": 0.0, "shield": 0.0}
...
func _tick_breakout(delta):
    _tick_breakout_powerups(delta)
    var v = _move_vec()
    paddle_x = clamp(paddle_x + v.x * 520 * delta, map_w * 0.08, map_w * 0.92)
    if breakout_power.get("laser", 0.0) > 0.0:
        player["cool"] = max(0.0, player["cool"] - delta)
        if _action() and player["cool"] <= 0.0:
            bullets.append({"pos": Vector2(paddle_x - paddle_w * 0.34, map_h * 0.86), "vel": Vector2(0, -620), "ttl": 1.1, "kind": "laser"})
            bullets.append({"pos": Vector2(paddle_x + paddle_w * 0.34, map_h * 0.86), "vel": Vector2(0, -620), "ttl": 1.1, "kind": "laser"})
            player["cool"] = 0.2
    ...
    if ball["pos"].y < 8:
        ball["vel"].y *= -1
    var paddle = Rect2(paddle_x - paddle_w * 0.5, map_h * 0.87, paddle_w, 14)
    if paddle.has_point(ball["pos"]) and ball["vel"].y > 0:
        ball["vel"].y = -abs(ball["vel"].y) - 8
        ball["vel"].x += (ball["pos"].x - paddle_x) * 4
    if ball["pos"].y > map_h and breakout_power.get("shield", 0.0) > 0.0:
        ball["pos"].y = map_h - 24
        ball["vel"].y = -abs(ball["vel"].y)
        breakout_power["shield"] = max(0.0, float(breakout_power["shield"]) - 2.5)
        _burst(ball["pos"], C_CYAN, 14)
    elif ball["pos"].y > map_h:
        _lose_life()
        ball = {"pos": Vector2(map_w * 0.5, map_h * 0.68), "vel": Vector2(210, -260)}
    for i in range(bricks.size() - 1, -1, -1):
        if bricks[i]["rect"].grow(6).has_point(ball["pos"]):
            ...
            bricks[i]["hp"] -= 1
            if bricks[i]["hp"] <= 0:
                var drop_pos = bricks[i]["rect"].get_center()
                _burst(drop_pos, C_MAGENTA, 12)
                if randf() < 0.42:
                    var kinds = ["wide", "slow", "laser", "shield", "life", "bonus"]
                    items.append({"pos": drop_pos, "vel": Vector2(0, randf_range(105, 150)), "kind": kinds[randi() % kinds.size()]})
                bricks.remove_at(i)
                score += 50
                _emit_score()
            break
    for i in range(items.size() - 1, -1, -1):
        var item = items[i]
        item["pos"] += item.get("vel", Vector2(0, 120)) * delta
        if paddle.grow(10).has_point(item["pos"]):
            var kind = str(item.get("kind", "bonus"))
            if kind == "wide":
                breakout_power["wide"] = 10.0
                paddle_w = 230.0
            elif kind == "slow":
                breakout_power["slow"] = 8.0
                if ball["vel"].length() > 260.0:
                    ball["vel"] = ball["vel"].normalized() * 260.0
            elif kind == "laser":
                breakout_power["laser"] = 9.0
            elif kind == "shield":
                breakout_power["shield"] = 11.0
            elif kind == "life":
                lives = min(9, lives + 1)
            else:
                score += 250
                _emit_score()
            _burst(item["pos"], C_YELLOW, 12)
            items.remove_at(i)
        elif item["pos"].y > map_h + 40:
            items.remove_at(i)
    if bricks.is_empty():
        wave += 1
        _setup_breakout()

func _tick_breakout_powerups(delta):
    for k in breakout_power.keys():
        breakout_power[k] = max(0.0, float(breakout_power[k]) - delta)
    paddle_w = 230.0 if breakout_power.get("wide", 0.0) > 0.0 else 140.0
    if breakout_power.get("slow", 0.0) > 0.0 and ball["vel"].length() > 280.0:
        ball["vel"] = ball["vel"].normalized() * 280.0
    for bi in range(bullets.size() - 1, -1, -1):
        if bullets[bi].get("kind", "") != "laser":
            continue
        for i in range(bricks.size() - 1, -1, -1):
            if bricks[i]["rect"].grow(4).has_point(bullets[bi]["pos"]):
                bricks[i]["hp"] -= 1
                _burst(bullets[bi]["pos"], C_YELLOW, 8)
...
func _draw_breakout():
    ...
    for item in items:
        ...
        if kind == "slow":
            ...
            col = C_YELLOW
        _glow_circle_outline(item["pos"], 11, col, 2)
    _glow_line(Vector2(paddle_x - paddle_w * 0.5, map_h * 0.87), Vector2(paddle_x + paddle_w * 0.5, map_h * 0.87), C_CYAN, 5)
    if breakout_power.get("shield", 0.0) > 0.0:
        _glow_line(Vector2(map_w * 0.08, map_h * 0.96), Vector2(map_w * 0.92, map_h * 0.96), C_ORANGE, 2.0)
...
func _draw_hud():
    ...
    if game_id == "marble_madness":
        ...
        var active = []
        for k in breakout_power.keys():
            if breakout_power[k] > 0.0:
                active.append(k.to_upper() + " " + str(int(ceil(breakout_power[k]))))
