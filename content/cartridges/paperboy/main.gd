...
var bounds_clamp = true
var lane_width = 1.0
var lane_spacing = 1.0
var shoulder_width = 1.0
var safe_zone_padding = 1.0
var house_setback = 1.0
var house_spacing = 1.0
var yard_depth = 1.0
var delivery_side_bias = 0.0
var subscriber_ratio = 0.68
...
func _ready():
    ...
    if shared_loader_script == null:
        push_error(game_title + " could not load app/shared/shared_loader.gd")
    ...
    if screenshot_path != "":
        ...
        if img == null:
            print(game_title, " screenshot skipped: viewport image unavailable for this display driver")
        else:
            var err = img.save_png(screenshot_path)
            print(game_title, " screenshot saved: ", screenshot_path, " err=", err)
        get_tree().quit()
...
func _setup_shared_tab_menu():
    ...
    if tab_script == null:
        push_error(game_title + " could not load shared TabMenu")
        return
    ...
    tab_menu.register_knob_float("grid_scale", "Grid Scale", grid_scale, 0.5, 2.0, 0.1, "Collision")
    tab_menu.register_knob_float("lane_width", "Lane Width", lane_width, 0.7, 1.6, 0.05, "Collision")
    tab_menu.register_knob_float("lane_spacing", "Lane Spacing", lane_spacing, 0.7, 1.6, 0.05, "Collision")
    tab_menu.register_knob_float("shoulder_width", "Shoulder Width", shoulder_width, 0.4, 1.8, 0.05, "Collision")
    tab_menu.register_knob_float("safe_zone_padding", "Safe-Zone Padding", safe_zone_padding, 0.4, 1.8, 0.05, "Collision")
    tab_menu.register_knob_bool("invert", "Invert Lanes", invert_lanes, "Collision")
    tab_menu.register_knob_bool("bounds_clamp", "Bounds Clamp", bounds_clamp, "Collision")
    tab_menu.register_knob_float("house_setback", "House Setback", house_setback, 0.4, 1.8, 0.05, "Secondary")
    tab_menu.register_knob_float("house_spacing", "House Spacing", house_spacing, 0.6, 1.8, 0.05, "Secondary")
    tab_menu.register_knob_float("yard_depth", "Yard Depth", yard_depth, 0.4, 1.8, 0.05, "Secondary")
    tab_menu.register_knob_int("island_count", "Island Count", island_count, 3, 6, 1, "Secondary")
    tab_menu.register_knob_float("island_width", "Island Width", island_width, 0.7, 1.8, 0.05, "Secondary")
    tab_menu.register_knob_float("island_depth", "Island Depth", island_depth, 0.7, 1.8, 0.05, "Secondary")
    tab_menu.register_knob_float("delivery_side_bias", "Delivery Side Bias", delivery_side_bias, -1.0, 1.0, 0.1, "Secondary")
    tab_menu.register_knob_float("subscriber_ratio", "Subscriber Ratio", subscriber_ratio, 0.25, 0.95, 0.05, "Secondary")
...
func _apply_shared_menu_settings():
    ...
    lane_width = float(tab_menu.get_knob_value("lane_width"))
    lane_spacing = float(tab_menu.get_knob_value("lane_spacing"))
    shoulder_width = float(tab_menu.get_knob_value("shoulder_width"))
    safe_zone_padding = float(tab_menu.get_knob_value("safe_zone_padding"))
    ...
    house_setback = float(tab_menu.get_knob_value("house_setback"))
    house_spacing = float(tab_menu.get_knob_value("house_spacing"))
    yard_depth = float(tab_menu.get_knob_value("yard_depth"))
    island_count = int(tab_menu.get_knob_value("island_count"))
    island_width = float(tab_menu.get_knob_value("island_width"))
    island_depth = float(tab_menu.get_knob_value("island_depth"))
    delivery_side_bias = float(tab_menu.get_knob_value("delivery_side_bias"))
...
func _dismiss_cover_art():
    if splash_rect != null and is_instance_valid(splash_rect):
...
func _load_lane_adapter_grid() -> bool:
    if shared_loader_script == null:
        return false
    if lane_adapter_script == null:
        lane_adapter_script = shared_loader_script.load_adapter_script("lane")
    if lane_adapter_script == null:
        return false
    var adapter = lane_adapter_script.new()
    var knobs = {
    ...
    if typeof(lanes) != TYPE_ARRAY or lanes.is_empty():
        return false
    var lane_h = max(16.0, float(layout.get("lane_height", 64.0)) * grid_scale)
    var bounds = layout.get("bounds", Rect2(0, 0, 1280, 720))
    cell_px = lane_h
    grid_w = max(8, int(ceil(bounds.size.x / cell_px)))
    grid_h = max(8, lanes.size())
    grid_cells = []
    for y in range(grid_h):
        var row = []
        var lane_type = "safe"
        if y < lanes.size():
            lane_type = str(lanes[y].get("type", "safe"))
    ...
    map_w = max(640.0, bounds.size.x)
    map_h = max(480.0, bounds.size.y)
    print(game_title, " SharedLoader LaneAdapter level=", level_dir, " grid=", grid_w, "x", grid_h, " lanes=", lanes.size())
    return true
...
func _setup_paperboy():
    ...
    scroll_y = 0.0
    paperboy_safe_timer = 2.25
    house_islands.clear()
    house_batches.clear()
    var road_center = map_w * 0.5
    var road_half = _paperboy_road_half_width()
    var shoulder_px = cell_px * (0.55 + shoulder_width * 0.9)
    house_islands = _build_paperboy_islands(road_center, road_half, shoulder_px)
    var batch_count = max(8, island_count * 2)
    var houses_per_batch = max(2, int(round(1.0 + traffic_density * 2.0)))
    var route_step = cell_px * (1.4 + house_spacing * 1.3)
    var batch_spacing_scroll = map_h * (0.9 + lane_spacing * 0.28)
    for batch_idx in range(batch_count):
        var member_indices = []
        for _j in range(houses_per_batch):
            var house_idx = houses.size()
            houses.append({
                "pos": Vector2.ZERO,
                "subscriber": true,
                "hit": false,
                "side": -1.0,
                "mailbox": Vector2.ZERO,
                "start_scroll": 0.0,
                "island_id": 0
            })
            member_indices.append(house_idx)
        house_batches.append({
            "members": member_indices,
            "start_scroll": 0.0,
            "island_id": batch_idx % max(1, house_islands.size())
        })
        var start_scroll = batch_idx * batch_spacing_scroll
        if batch_idx == 0:
            start_scroll = -map_h * 0.72
        elif batch_idx == 1:
            start_scroll = -map_h * 0.18
        _assign_house_batch(batch_idx, start_scroll, batch_idx % max(1, house_islands.size()))
    var route_depth = batch_count * batch_spacing_scroll + map_h
    var hazard_count = max(12, int(round(route_depth / max(54.0, route_step * 0.85) * traffic_density)))
    for i in range(hazard_count):
        var lane_t = randf_range(-0.58, 0.58)
        var start_y = -80.0 - i * route_step * 0.82
        road_markers.append({
            "pos": Vector2(road_center + lane_t * road_half * 0.9, start_y),
            "radius": randf_range(6.0, 10.0)
        })
...
func _process(delta):
    ...
    if blanked:
        return
    if _shared_menu_open() or paused:
        queue_redraw()
        return
...
func _tick_paperboy(delta):
    ...
    paperboy_safe_timer = max(0.0, paperboy_safe_timer - delta)
    _refresh_house_batches()
    var move = _input_vector(0)
    if abs(move.x) > 0.1:
    ...
    for pa_i in range(papers.size() - 1, -1, -1):
        papers[pa_i]["vel"].y += 190.0 * delta
        papers[pa_i]["pos"] += papers[pa_i]["vel"] * delta
        papers[pa_i]["life"] += delta
        var out = papers[pa_i]["pos"].x < -160.0 or papers[pa_i]["pos"].x > map_w + 160.0 or papers[pa_i]["pos"].y < -160.0 or papers[pa_i]["pos"].y > map_h + 180.0
        if out and papers[pa_i]["life"] > 0.1:
            papers.remove_at(pa_i)
    for h in houses:
        var hp = _paperboy_house_world_pos(h)
        var mailbox = _paperboy_mailbox_world_pos(h)
        if hp.x < -90000.0:
            continue
        for pi in range(papers.size() - 1, -1, -1):
            if mailbox.distance_to(papers[pi]["pos"]) < 34.0 or hp.distance_to(papers[pi]["pos"]) < 44.0:
                if h["subscriber"] and not h["hit"]:
                    score += 100
                    h["hit"] = true
                    _emit_score(1)
                    _burst(mailbox, NEON_GREEN, 14)
                elif not h["subscriber"]:
                    score = max(0, score - 50)
                    _emit_score(1)
                    _burst(mailbox, NEON_RED, 10)
                papers.remove_at(pi)
                break
    for m in road_markers:
        var rp = m["pos"] + Vector2(0, scroll_y)
        if rp.y > map_h + 40:
            m["pos"].y -= max(map_h * 2.0, road_markers.size() * cell_px * 1.4)
            m["pos"].x = map_w * 0.5 + randf_range(-0.58, 0.58) * road_half * 0.9
        if paperboy_safe_timer <= 0.0 and rp.distance_to(p["pos"]) < float(m.get("radius", 8.0)) + 14.0:
            _lose_life()
...
func _draw_paperboy():
    ...
    var road_half = _paperboy_road_half_width()
    var shoulder_px = cell_px * (0.55 + shoulder_width * 0.9)
    var curb_half = road_half + shoulder_px
    draw_polygon(
        PackedVector2Array([
            Vector2(road_x - road_half, 0),
            Vector2(road_x + road_half, 0),
            Vector2(road_x + curb_half + 34.0, map_h),
            Vector2(road_x - curb_half - 34.0, map_h)
        ]),
        PackedColorArray([Color(0, 0.9, 1, 0.08)])
    )
    draw_line(Vector2(road_x - road_half, 0), Vector2(road_x - curb_half - 34.0, map_h), Color(1, 1, 1, 0.18), 1.0)
    draw_line(Vector2(road_x + road_half, 0), Vector2(road_x + curb_half + 34.0, map_h), Color(1, 1, 1, 0.18), 1.0)
    _draw_paperboy_house_islands()
    for h in houses:
        var hp = _paperboy_house_world_pos(h)
        var mailbox = _paperboy_mailbox_world_pos(h)
        if hp.x < -90000.0:
        ...
        if hp.y > -80 and hp.y < map_h + 80:
            ...
            if h["hit"]:
                col = NEON_YELLOW
            if _paperboy_house_in_flash_window(h):
...
func _build_paperboy_islands(road_center: float, road_half: float, shoulder_px: float) -> Array:
    var result = []
    var outer_gap = cell_px * (1.0 + house_setback * 1.1)
    var usable_top = map_h * clamp(0.44 + island_depth * 0.12, 0.42, 0.68)
    var top_margin = map_h * 0.04
    var left_min = cell_px * 0.8
    var left_max = road_center - road_half - shoulder_px - outer_gap
    var right_min = road_center + road_half + shoulder_px + outer_gap
    var right_max = map_w - cell_px * 0.8
    var left_span = max(96.0, left_max - left_min)
    var right_span = max(96.0, right_max - right_min)
    var side_width = max(80.0, min(left_span, right_span) * (0.55 + island_width * 0.22))
    var count = clampi(island_count, 3, 6)
    for i in range(count):
        var side = -1.0 if (i % 2 == 0) else 1.0
        var lane = float(i) / max(1.0, float(count - 1))
        var island_h = max(84.0, usable_top * (0.22 + island_depth * 0.08))
        var island_y = lerpf(top_margin, usable_top - island_h, lane)
        var center_x = left_min + left_span * 0.5 if side < 0.0 else right_max - right_span * 0.5
        if count >= 5 and i == 0:
            island_h *= 1.15
        result.append({
            "rect": Rect2(center_x - side_width * 0.5, island_y, side_width, island_h),
            "side": side
        })
    if result.is_empty():
        result.append({"rect": Rect2(32, top_margin + 24.0, 140, 160), "side": -1.0})
        result.append({"rect": Rect2(map_w - 172.0, top_margin + 100.0, 140, 160), "side": 1.0})
    return result

func _assign_house_batch(batch_idx: int, start_scroll: float, island_id: int):
    if batch_idx < 0 or batch_idx >= house_batches.size() or house_islands.is_empty():
        return
    var batch = house_batches[batch_idx]
    var island = house_islands[clampi(island_id, 0, house_islands.size() - 1)]
    batch["start_scroll"] = start_scroll
    batch["island_id"] = island_id
    var rect: Rect2 = island["rect"]
    var side = float(island["side"])
    var road_center = map_w * 0.5
    var road_half = _paperboy_road_half_width()
    var shoulder_px = cell_px * (0.55 + shoulder_width * 0.9)
    ...
    for local_idx in range(members.size()):
        ...
        if house_idx < 0 or house_idx >= houses.size():
            continue
        var lane_pull = randf_range(-1.0, 1.0) * target_spread * cell_px * 0.45
        var spawn_x = randf_range(rect.position.x + 24.0, rect.end.x - 24.0)
        var spawn_y = randf_range(rect.position.y + 36.0, rect.end.y - 18.0)
        var mailbox_x = road_center + side * (road_half + shoulder_px + yard_px * 0.45 + lane_pull)
        var mailbox_y = spawn_y + randf_range(-8.0, 10.0)
        houses[house_idx] = {
            "pos": Vector2(spawn_x, spawn_y),
            "subscriber": randf() < subscriber_ratio,
            "hit": false,
            "side": side,
            "mailbox": Vector2(mailbox_x, mailbox_y),
            "start_scroll": start_scroll,
            "island_id": island_id
        }
    house_batches[batch_idx] = batch

func _paperboy_house_world_pos(h: Dictionary) -> Vector2:
    var start_scroll = float(h.get("start_scroll", 0.0))
    var travel = scroll_y - start_scroll
    if travel < -180.0:
        return Vector2(-99999.0, -99999.0)
    return Vector2(h["pos"].x, h["pos"].y + max(0.0, travel))

func _paperboy_mailbox_world_pos(h: Dictionary) -> Vector2:
    var start_scroll = float(h.get("start_scroll", 0.0))
    var travel = scroll_y - start_scroll
    if travel < -180.0:
        return Vector2(-99999.0, -99999.0)
    return Vector2(h["mailbox"].x, h["mailbox"].y + max(0.0, travel))

func _paperboy_house_in_flash_window(h: Dictionary) -> bool:
    var remaining = float(h.get("start_scroll", 0.0)) - scroll_y
    return remaining >= 0.0 and remaining <= 170.0

func _paperboy_flash_on(h: Dictionary) -> bool:
    var remaining = float(h.get("start_scroll", 0.0)) - scroll_y
    return int(floor((170.0 - remaining) / 24.0)) % 2 == 0

func _refresh_house_batches():
    if house_batches.is_empty() or house_islands.is_empty():
        return
    var max_start = scroll_y
    for batch in house_batches:
        max_start = max(max_start, float(batch.get("start_scroll", 0.0)))
    var respawn_gap = map_h * (0.9 + lane_spacing * 0.28)
    for batch_idx in range(house_batches.size()):
        var batch = house_batches[batch_idx]
        var start_scroll = float(batch.get("start_scroll", 0.0))
        if scroll_y <= start_scroll + map_h + 220.0:
            continue
        max_start += respawn_gap
        var next_island = randi() % max(1, house_islands.size())
        _assign_house_batch(batch_idx, max_start, next_island)

func _draw_paperboy_house_islands():
    for batch in house_batches:
        var start_scroll = float(batch.get("start_scroll", 0.0))
        var remaining = start_scroll - scroll_y
        if remaining < 0.0 or remaining > 170.0:
            continue
        var island_id = clampi(int(batch.get("island_id", 0)), 0, max(0, house_islands.size() - 1))
        var rect: Rect2 = house_islands[island_id]["rect"]
        var blink_on = int(floor((170.0 - remaining) / 24.0)) % 2 == 0
        var alpha = 0.16 if blink_on else 0.04
        draw_rect(rect, Color(1.0, 0.84, 0.2, alpha), true)
        draw_rect(rect, Color(1.0, 0.9, 0.35, 0.65 if blink_on else 0.22), false, 2.0)

func _paperboy_marker_masked(pos: Vector2) -> bool:
    for batch in house_batches:
        var start_scroll = float(batch.get("start_scroll", 0.0))
        var remaining = start_scroll - scroll_y
        if remaining < 0.0 or remaining > 170.0:
        ...
        var island_id = clampi(int(batch.get("island_id", 0)), 0, max(0, house_islands.size() - 1))
        var rect: Rect2 = house_islands[island_id]["rect"]
        if rect.grow(6.0).has_point(pos):
            return true
    return false
...
func _draw_house(pos: Vector2, color: Color, scale: float = 1.0):
    var body = Rect2(pos - Vector2(28, 12) * scale, Vector2(56, 38) * scale)
    var roof = PackedVector2Array([pos + Vector2(-34, -12) * scale, pos + Vector2(0, -38) * scale, pos + Vector2(34, -12) * scale])
    ...
    draw_rect(Rect2(pos + Vector2(-6, 4) * scale, Vector2(12, 22) * scale), Color(color.r, color.g, color.b, 0.22), true)

func _draw_bicycle_rider(pos: Vector2, color: Color, scale: float = 1.0):
    _glow_circle_outline(pos + Vector2(-13, 15) * scale, 9.0 * scale, color, 1.3)
    _glow_circle_outline(pos + Vector2(13, 15) * scale, 9.0 * scale, color, 1.3)
    _glow_line(pos + Vector2(-13, 15) * scale, pos + Vector2(0, -2) * scale, color, 1.2)
    _glow_line(pos + Vector2(13, 15) * scale, pos + Vector2(0, -2) * scale, color, 1.2)
    _glow_line(pos + Vector2(-4, -8) * scale, pos + Vector2(8, -8) * scale, color, 1.2)
    _draw_human(pos + Vector2(0, -18) * scale, color, 0.55 * scale)
