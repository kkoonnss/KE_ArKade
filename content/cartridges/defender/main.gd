...
func _setup_missile_command():
    ...
    for x in [0.08, 0.5, 0.92]:
        silos.append({"pos": Vector2(map_w * x, map_h * 0.92), "ammo": 22})

func _setup_defender():
    defender_facing = 1.0
    _setup_player(Vector2(map_w * 0.5, map_h * 0.42))
    for i in range(max(5, 11 - int(wave / 2))):
        pickups.append({"pos": Vector2(randf() * map_w, map_h * randf_range(0.46, 0.68)), "rescued": false})
    for i in range(min(22, 7 + wave * 2)):
        enemies.append({"pos": Vector2(randf() * map_w, map_h * randf_range(0.12, 0.54)), "speed": randf_range(85, 145) + wave * 12.0, "alive": true, "carry": -1, "target_index": i % max(1, pickups.size()), "juke": randf() * TAU})
...
func _tick_defender(delta):
    ...
    p["cooldown"] = max(0.0, p["cooldown"] - delta)
    if _shoot_pressed(0) and p["cooldown"] <= 0.0:
        bullets.append({"pos": p["pos"], "vel": Vector2(620.0 * defender_facing, 0), "ttl": 1.0})
        p["cooldown"] = 0.16
    _tick_bullets(delta, true)
    for e in enemies:
        var target = p["pos"]
        if pickups.size() > 0 and e["carry"] == -1:
            var idx = int(e.get("target_index", 0)) % pickups.size()
            target = pickups[idx]["pos"] + Vector2(0, -26)
        e["juke"] = float(e.get("juke", 0.0)) + delta * (1.5 + wave * 0.12)
        var chase = (target - e["pos"]).normalized()
        var weave = Vector2(-chase.y, chase.x) * sin(float(e["juke"])) * min(0.55, 0.12 + wave * 0.035)
        e["pos"] = _wrap_world(e["pos"] + (chase + weave).normalized() * e["speed"] * delta)
        e["pos"].y = clamp(e["pos"].y, cell_px, map_h * 0.70)
...
func _draw_centipede():
    for b in barriers:
        _glow_circle(b["pos"], 11.0, NEON_GREEN)
    for b in bullets:
        _glow_line(b["pos"] + Vector2(0, 12), b["pos"] - Vector2(0, 12), NEON_YELLOW, 2.0)
    for i in range(centipede.size()):
        _glow_circle(centipede[i]["pos"], 15.0, NEON_MAGENTA if i == 0 else NEON_ORANGE)
...
func _draw_robotron_2084():
    for h in pickups:
        _glow_circle(h["pos"], 10.0, NEON_GREEN)
    for e in enemies:
        _glow_circle(e["pos"], 14.0, NEON_RED)
    for b in bullets:
        _glow_circle(b["pos"], 4.0, NEON_YELLOW)
    _glow_circle(players[0]["pos"], 17.0, NEON_CYAN)
...
func _draw_burger():
    ...
    for ing in ingredients:
        ...
        draw_rect(ing["rect"], Color(col.r, col.g, col.b, 0.22), true)
        draw_rect(ing["rect"], col, false, 2.0)
    for e in enemies:
        _glow_circle(e["pos"], 13.0, NEON_ORANGE)
    _glow_circle(players[0]["pos"], 13.0, NEON_CYAN)

func _draw_missile_command():
    for c in cities:
        var col = NEON_GREEN if c["alive"] else INK_DIM
        draw_rect(Rect2(c["pos"] - Vector2(22, 18), Vector2(44, 18)), Color(col.r, col.g, col.b, 0.2), true)
        draw_rect(Rect2(c["pos"] - Vector2(22, 18), Vector2(44, 18)), col, false, 1.5)
    for s in silos:
...
func _draw_defender():
    ...
    for h in pickups:
        _glow_circle(h["pos"], 8.0, NEON_GREEN)
    for e in enemies:
        _draw_alien(e["pos"], NEON_MAGENTA)
    for b in bullets:
        _glow_line(b["pos"] - Vector2(12, 0), b["pos"] + Vector2(12, 0), NEON_YELLOW, 2.0)
    _draw_player_ship(players[0]["pos"], NEON_CYAN, true, defender_facing)
...
func _draw_paperboy():
    ...
    for h in houses:
        ...
        if hp.y > -80 and hp.y < map_h + 80:
            ...
            if h["hit"]:
                col = NEON_YELLOW
            draw_rect(Rect2(hp - Vector2(28, 24), Vector2(56, 48)), Color(col.r, col.g, col.b, 0.18), true)
            draw_rect(Rect2(hp - Vector2(28, 24), Vector2(56, 48)), col, false, 1.5)
    for m in road_markers:
        var rp = m["pos"] + Vector2(0, scroll_y)
    ...
    _glow_circle(players[0]["pos"], 14.0, NEON_CYAN)
...
func _draw_qbert():
    ...
    _glow_circle(_cube_screen(cube_player) + Vector2(0, -28), 13.0, NEON_GREEN)
    for e in enemies:
        _glow_circle(_cube_screen(e["rc"]) + Vector2(0, -26), 11.0, NEON_RED)
...
func _safe_position(pos: Vector2) -> Vector2:
    var cell = _find_nearest_walkable_cell(_pos_to_cell(pos))
...
func _draw_player_ship(pos: Vector2, color: Color, horizontal := false, facing := 1.0):
    var pts: PackedVector2Array
    if horizontal:
        var f = -1.0 if facing < 0.0 else 1.0
        pts = PackedVector2Array([pos + Vector2(24 * f, 0), pos + Vector2(-18 * f, -14), pos + Vector2(-9 * f, 0), pos + Vector2(-18 * f, 14)])
    else:
...
func _draw_alien(pos: Vector2, color: Color):
    _glow_circle_outline(pos, 16.0, color, 2.0)
    _glow_line(pos + Vector2(-20, 6), pos + Vector2(20, 6), color, 1.5)
