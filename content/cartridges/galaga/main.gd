...
func _setup_shared_tab_menu():
    ...
    shared_tab_menu.setup("galaga", level_dir, "GALAGA")
...
func _setup_invaders():
    _setup_player(_galaga_player_spawn() if game_id == "galaga" else Vector2(map_w * 0.5, map_h * 0.88))
    var rows = maxi(3, int(round((5 if game_id == "galaga" else 4) * (arena_density if game_id == "galaga" else 1.0))))
    var cols = maxi(6, int(round(10 * (min(1.4, arena_density) if game_id == "galaga" else 1.0))))
    var form_bounds = arena_bounds if game_id == "galaga" else Rect2(0, 0, map_w, map_h)
    for y in range(rows):
        for x in range(cols):
            ...
            var entry_side = -1 if x < cols / 2 else 1
            var entry_pos = Vector2(form_bounds.get_center().x + entry_side * form_bounds.size.x * 0.58, form_bounds.position.y - 40 - y * 26 - x * 8)
            enemies.append({
                "pos": entry_pos if game_id == "galaga" else base_pos,
                "base": base_pos,
                "kind": "boss" if game_id == "galaga" and y == 0 and x in [4, 5] else "alien",
                "phase": randf() * TAU,
                "cooldown": randf_range(1.0, 3.0),
                "alive": true,
                "state": "enter" if game_id == "galaga" else "formation",
                "dive": 0.0,
                "maneuver": randf() < 0.4,
                "maneuver_type": randi() % 3,
                "maneuver_time": 0.0,
...
func _tick_invaders(delta):
    if game_id == "galaga":
        star_scroll = fmod(star_scroll + delta * (28.0 + wave * 3.0), 96.0)
    var p = players[0]
    var move = _input_vector(0)
    ...
    for e in enemies:
        if not e["alive"]:
            continue
        if game_id == "galaga":
            e["phase"] += delta * (2.0 + wave * 0.08)
            var state = str(e.get("state", "formation"))
            if state == "enter":
                var arc = Vector2(sin(e["phase"]) * 95.0 * float(e.get("entry_side", 1)), cos(e["phase"] * 0.7) * 34.0)
                e["pos"] = e["pos"].lerp(e["base"] + arc * 0.08, min(1.0, delta * 1.45))
                if e["pos"].distance_to(e["base"]) < 14.0:
                    e["state"] = "formation"
                    e["pos"] = e["base"]
            elif state == "formation":
                e["pos"] = e["base"] + Vector2(sin(e["phase"]) * 10.0, cos(e["phase"] * 0.7) * 5.0)
                e["cooldown"] = float(e.get("cooldown", 1.0)) - delta
                var active_divers = _galaga_active_divers()
                var dive_budget = 1 if wave < 3 else (2 if wave < 6 else 3)
                if e["cooldown"] <= 0.0 and active_divers < dive_budget and randf() < delta * (0.34 + wave * 0.025):
                    if bool(e.get("maneuver", false)) and randf() < 0.45:
                        e["state"] = "maneuver"
                        e["maneuver_time"] = 0.0
                        e["cooldown"] = randf_range(2.3, 4.0)
                    else:
                        e["state"] = "dive"
                        e["dive"] = 0.0
                        e["cooldown"] = randf_range(2.8, 5.2)
            elif state == "dive":
                e["dive"] = float(e.get("dive", 0.0)) + delta
                var to_player = (p["pos"] - e["pos"]).normalized()
                var swoop = Vector2(sin(e["phase"] * 2.2), 0.25).normalized() * 78.0
                var lead = Vector2(sign(p["pos"].x - e["pos"].x) * 54.0, 24.0 + sin(e["phase"] * 1.6) * 16.0)
                e["pos"] += (to_player * (150.0 + wave * 14.0) + swoop + lead) * delta
                if randf() < delta * (0.7 + wave * 0.08):
                    enemy_bullets.append({"pos": e["pos"], "vel": (p["pos"] - e["pos"]).normalized() * (250 + wave * 18), "ttl": 2.1})
                if e["pos"].y > map_h + 40 or e["dive"] > 3.8:
                    e["state"] = "return"
            elif state == "maneuver":
                e["maneuver_time"] = float(e.get("maneuver_time", 0.0)) + delta
                var t = float(e["maneuver_time"])
                var pos = Vector2(e["base"].x, e["base"].y)
                match int(e.get("maneuver_type", 0)):
                    0:
                        pos += Vector2(sin(t * 4.0 + e["phase"]) * 82.0, cos(t * 2.4 + e["phase"]) * 22.0 - 18.0)
                    1:
                        pos += Vector2(cos(t * 3.1 + e["phase"]) * 62.0, sin(t * 3.1 + e["phase"]) * 30.0 - 22.0)
                    _:
                        pos += Vector2(sin(t * 2.0 + e["phase"]) * 96.0, sin(t * 4.4 + e["phase"]) * 18.0)
                        pos.y -= abs(sin(t * 2.0 + e["phase"])) * 22.0
                pos.y = min(pos.y, e["base"].y - 12.0 + sin(t * 3.6 + e["phase"]) * 8.0)
                e["pos"] = pos
                if t > randf_range(2.0, 3.4):
                    e["state"] = "return"
            elif state == "return":
                e["pos"] = e["pos"].lerp(e["base"], min(1.0, delta * 1.9))
                if e["pos"].distance_to(e["base"]) < 16.0:
                    e["state"] = "formation"
                    e["pos"] = e["base"]
