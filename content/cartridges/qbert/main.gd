...
func _build_shell():
    ...
    for line in [
        game_title,
        "TAB: SETTINGS / F1: REFERENCE / ENTER: RESTART",
        "WASD OR ARROWS: MOVE / SPACE OR CLICK: FIRE",
        "IPC: LOAD PAUSE RESUME BLANK QUIT READY SCORE HEARTBEAT"
    ]:
        var label = Label.new()
        label.text = line
...
func _process_splash(delta):
    if splash_rect and splash_timer > 0.0:
        splash_timer -= delta
        if splash_timer <= 0.0:
            splash_rect.queue_free()
            splash_rect = null
        elif splash_timer < 0.8:
            splash_rect.modulate.a = splash_timer / 0.8
