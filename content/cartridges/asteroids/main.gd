...
var map_w = 1280.0
var map_h = 720.0
var base_cell_px = 32.0
var cell_px = 32.0
var cell_size = Vector2(32.0, 32.0)
...
var reference_opacity = 0.3
var background_view = "final"
var secondary_strength = 0.75
var collision_mode = "grid"
var grid_resolution = 1.0
var grid_expand_px = 0.0
var lane_expand_px = 0.0
var invert_grid = false
var invert_lanes = false
var organic_boundary = false
var organic_rounding_px = 18.0
var show_debug_grid = true
var ship_scale = 0.65
var secondary_lane_alpha = 0.24
var secondary_fill_alpha = 0.32
var secondary_outline_px = 2
var secondary_photo_mix = 0.18
var current_cover_blocks = []
var organic_mask_image: Image = null
var collision_preview_texture: Texture2D = null

var ui_canvas: CanvasLayer
var tab_menu = null
var shared_loader_script = null
var arena_adapter_script = null
...
func _ready():
    ...
    if shared_loader_script == null:
        push_error(title + " could not load app/shared/shared_loader.gd")
    ...
    if screenshot_path != "":
        ...
        if img == null:
            print(title, " screenshot skipped: viewport image unavailable for this display driver")
        else:
            var err = img.save_png(screenshot_path)
            print(title, " screenshot saved: ", screenshot_path, " err=", err)
        get_tree().quit()
...
func _setup_shared_tab_menu():
    ...
    if tab_script == null:
        push_error(title + " could not load shared TabMenu")
        return
    ...
    tab_menu.register_knob_enum("collision_mode", "Collision Mode", collision_mode, ["grid", "organic"], "Collision")
    tab_menu.register_knob_bool("organic_boundary", "Organic Boundary", organic_boundary, "Collision")
    tab_menu.register_knob_float("grid_resolution", "Grid Resolution", grid_resolution, 0.5, 3.0, 0.25, "Collision")
    tab_menu.register_knob_float("grid_expand_px", "Grid Expand", grid_expand_px, -48.0, 48.0, 4.0, "Collision")
    tab_menu.register_knob_float("lane_expand_px", "Lane Expand", lane_expand_px, -48.0, 48.0, 4.0, "Collision")
    tab_menu.register_knob_bool("invert_grid", "Invert Grid", invert_grid, "Collision")
    tab_menu.register_knob_bool("invert_lanes", "Invert Lanes", invert_lanes, "Collision")
    tab_menu.register_knob_float("organic_rounding_px", "Organic Rounding", organic_rounding_px, 4.0, 48.0, 2.0, "Collision")
    tab_menu.register_knob_bool("bounds_clamp", "Bounds Clamp", true, "Collision")
    tab_menu.register_knob_bool("block_region", "Central Block", false, "Collision")
    tab_menu.register_knob_float("secondary_strength", "Secondary Strength", secondary_strength, 0.2, 1.5, 0.05, "Secondary")
    tab_menu.register_knob_float("secondary_lane_alpha", "Secondary Lanes", secondary_lane_alpha, 0.0, 0.7, 0.05, "Secondary")
    tab_menu.register_knob_float("secondary_fill_alpha", "Secondary Fill", secondary_fill_alpha, 0.0, 0.8, 0.05, "Secondary")
    tab_menu.register_knob_int("secondary_outline_px", "Secondary Outline", secondary_outline_px, 1, 8, 1, "Secondary")
    tab_menu.register_knob_float("ship_scale", "Ship Scale", ship_scale, 0.35, 1.25, 0.05, "Gameplay")
    ...
    tab_menu.setup(game_id, level_dir, title.to_upper())
...
func _apply_shared_menu_settings():
    ...
    collision_mode = str(tab_menu.get_knob_value("collision_mode"))
    organic_boundary = bool(tab_menu.get_knob_value("organic_boundary"))
    grid_resolution = float(tab_menu.get_knob_value("grid_resolution"))
    grid_expand_px = float(tab_menu.get_knob_value("grid_expand_px"))
    lane_expand_px = float(tab_menu.get_knob_value("lane_expand_px"))
    invert_grid = bool(tab_menu.get_knob_value("invert_grid"))
    invert_lanes = bool(tab_menu.get_knob_value("invert_lanes"))
    organic_rounding_px = float(tab_menu.get_knob_value("organic_rounding_px"))
    show_debug_grid = bool(tab_menu.get_knob_value("show_debug_grid"))
    secondary_strength = float(tab_menu.get_knob_value("secondary_strength"))
    secondary_lane_alpha = float(tab_menu.get_knob_value("secondary_lane_alpha"))
    secondary_fill_alpha = float(tab_menu.get_knob_value("secondary_fill_alpha"))
    secondary_outline_px = int(tab_menu.get_knob_value("secondary_outline_px"))
    secondary_photo_mix = float(tab_menu.get_knob_value("secondary_photo_mix"))
    ship_scale = float(tab_menu.get_knob_value("ship_scale"))

func _apply_classic_level_defaults():
    if tab_menu == null or not _is_classic_arcade_level():
        return
    tab_menu.set_knob_value("background_view", "final", false, false)
    tab_menu.set_knob_value("reference", false, false, false)
    tab_menu.set_knob_value("reference_opacity", 0.0, false, false)
    tab_menu.set_knob_value("secondary_photo_mix", 0.0, false, false)
    tab_menu.set_knob_value("show_debug_grid", false, false, false)
    tab_menu.set_knob_value("collision_mode", "grid", false, false)
    tab_menu.set_knob_value("organic_boundary", false, false, false)
    tab_menu.set_knob_value("grid_resolution", 1.0, false, false)
    tab_menu.set_knob_value("grid_expand_px", 0.0, false, false)
    tab_menu.set_knob_value("lane_expand_px", 0.0, false, false)
    tab_menu.set_knob_value("invert_grid", false, false, false)
    tab_menu.set_knob_value("invert_lanes", false, false, false)
    tab_menu.set_knob_value("secondary_strength", 0.0, false, false)
    tab_menu.set_knob_value("secondary_lane_alpha", 0.0, false, false)
    tab_menu.set_knob_value("secondary_fill_alpha", 0.0, false, false)
    tab_menu.set_knob_value("secondary_outline_px", 1, false, false)
    tab_menu.set_knob_value("ship_scale", 1.0, false, false)

func _on_shared_knob_changed(knob_id: String, value):
    _apply_shared_menu_settings()
    if knob_id in ["bounds_clamp", "block_region", "collision_mode", "organic_boundary", "grid_resolution", "grid_expand_px", "lane_expand_px", "invert_grid", "invert_lanes", "organic_rounding_px"]:
        load_level()
        reset_game()
    elif knob_id in ["secondary_strength", "secondary_lane_alpha", "secondary_fill_alpha", "secondary_outline_px", "secondary_photo_mix"]:
        _write_secondary_map()
        _load_secondary_texture()
        _rebuild_organic_collision_mask()
    elif knob_id in ["background_view", "reference", "reference_opacity", "show_debug_grid", "ship_scale"]:
        pass
    queue_redraw()

func _on_shared_menu_action(action_id: String):
    if action_id == "start" and state != "playing":
        reset_game()
...
func _build_ui():
    ...
    tab_menu.color = Color(0, 0, 0, 0.82)
...
func _update_menu_overlay():
    ...
    if overlay_mode == "help":
        ...
        lines = [
            ...
            "Action uses Space or click when the game supports it.",
            "Press Tab any time for in-game settings.",
            "F1 toggles the level reference overlay.",
            _menu_line(0),
            _menu_line(1)
        ]
...
func _menu_items_for_mode(mode: String) -> Array:
    if mode == "settings":
        return [
            {"label": "Players: " + str(selected_players), "action": "players"},
            {"label": "Reference Overlay: " + ("On" if show_reference else "Off"), "action": "reference"},
            {"label": "Restart Round", "action": "restart"},
...
func load_level():
    ...
    walkable.clear()
    var loaded = false
    if _load_classic_blank_arena():
        loaded = true
    elif _load_arena_adapter_grid():
        loaded = true
    elif level_dir != "":
    ...
    if level_dir != "":
        var source_size = _source_canvas_size()
        map_w = max(1.0, source_size.x if source_size.x > 0.0 else grid_w * cell_px)
        map_h = max(1.0, source_size.y if source_size.y > 0.0 else grid_h * cell_px)
        _load_reference_assets()
    else:
        map_w = max(cell_px, grid_w * cell_px)
        map_h = max(cell_px, grid_h * cell_px)
    cell_size = Vector2(map_w / max(1, grid_w), map_h / max(1, grid_h))
    _rebuild_walkable()
    _update_scale()

func _is_classic_arcade_level() -> bool:
    if game_id != "asteroids" or level_dir == "":
        return false
    var normalized = level_dir.replace("\\", "/").to_lower()
    var level_name = normalized.get_file()
    return level_name.begins_with("classic_") or "/scene_classic_pack/" in normalized

func _load_classic_blank_arena() -> bool:
    if not _is_classic_arcade_level():
        return false
    current_cover_blocks = []
    organic_mask_image = null
    collision_preview_texture = null
    reference_texture = null
    photo_texture = null
    semantic_texture = null
    secondary_texture = null
    cell_px = base_cell_px
    map_w = 1280.0
    map_h = 720.0
    grid_w = max(1, int(round(map_w / cell_px)))
    grid_h = max(1, int(round(map_h / cell_px)))
    cell_size = Vector2(map_w / max(1, grid_w), map_h / max(1, grid_h))
    grid = []
    for y in range(grid_h):
        ...
        for x in range(grid_w):
            row.append(2)
        grid.append(row)
    print(title, " classic blank arena level=", level_dir, " grid=", grid_w, "x", grid_h)
    return true

func _load_arena_adapter_grid() -> bool:
    if shared_loader_script == null:
        return false
    if arena_adapter_script == null:
        arena_adapter_script = shared_loader_script.load_adapter_script("arena")
    if arena_adapter_script == null:
        return false
    var adapter = arena_adapter_script.new()
    var knobs = {
        "bounds_clamp": true,
        "block_region": false
    }
    if tab_menu != null:
        ...
        knobs["block_region"] = tab_menu.get_knob_value("block_region")
    var layout = adapter.interpret(level_dir, {}, knobs)
    var bounds = layout.get("bounds", Rect2(0, 0, 1280, 720))
    var cover_blocks = layout.get("cover_blocks", [])
    current_cover_blocks = cover_blocks.duplicate(true)
    cell_px = base_cell_px / max(0.25, grid_resolution)
    var source_size = _source_canvas_size()
    map_w = max(cell_px, source_size.x if source_size.x > 0.0 else bounds.size.x)
    map_h = max(cell_px, source_size.y if source_size.y > 0.0 else bounds.size.y)
    grid_w = max(1, int(ceil(map_w / cell_px)))
    grid_h = max(1, int(ceil(map_h / cell_px)))
    cell_size = Vector2(map_w / max(1, grid_w), map_h / max(1, grid_h))
    grid = []
    for y in range(grid_h):
        var row = []
        for x in range(grid_w):
            row.append(1 if invert_grid else 2)
        grid.append(row)
    for block in cover_blocks:
        if not (block is Rect2):
            continue
        var grown = block.grow(grid_expand_px)
        var start_x = clampi(int(floor(grown.position.x / cell_size.x)), 0, grid_w - 1)
        var end_x = clampi(int(ceil((grown.position.x + grown.size.x) / cell_size.x)), 0, grid_w)
        var start_y = clampi(int(floor(grown.position.y / cell_size.y)), 0, grid_h - 1)
        var end_y = clampi(int(ceil((grown.position.y + grown.size.y) / cell_size.y)), 0, grid_h)
        for yy in range(start_y, end_y):
            for xx in range(start_x, end_x):
                grid[yy][xx] = 2 if invert_grid else 1
    if bool(knobs.get("block_region", false)):
        var cx = grid_w / 2
        var cy = grid_h / 2
        for yy in range(max(1, cy - 2), min(grid_h - 1, cy + 3)):
            for xx in range(max(1, cx - 3), min(grid_w - 1, cx + 4)):
                grid[yy][xx] = 1
    _rebuild_organic_collision_mask()
    _write_secondary_map()
    print(title, " SharedLoader ArenaAdapter level=", level_dir, " grid=", grid_w, "x", grid_h, " cover=", cover_blocks.size())
    return true
...
func _source_canvas_size() -> Vector2:
    if level_dir == "":
        return Vector2.ZERO
    for name in ["semantic_map.png", "background.jpeg", "background.png", "background.jpg"]:
        var path = level_dir.path_join(name)
        if FileAccess.file_exists(path):
            var img = Image.load_from_file(path)
            if img:
...
func _rebuild_walkable():
    walkable.clear()
    for y in range(grid_h):
        for x in range(grid[y].size()):
            var center = _cell_center(Vector2i(x, y))
            if not _point_blocked(center):
...
func _secondary_map_path() -> String:
    var edit_dir = _level_edit_dir()
    if edit_dir == "":
        return ""
    return edit_dir.path_join(game_id + ".secondary_map.png")

func _lane_cell(c: Vector2i) -> bool:
    var open = int(grid[c.y][c.x]) != 1
    return not open if invert_lanes else open

func _write_secondary_map():
    var edit_dir = _level_edit_dir()
    if edit_dir == "":
        return
    ...
    img.fill(Color(0.01, 0.03, 0.05, 1.0))
    var lane_color = Color(0.04, 0.78, 0.9, clamp(secondary_lane_alpha * secondary_strength, 0.0, 0.85))
    var cover_fill = Color(0.0, 0.95, 0.46, clamp(secondary_fill_alpha * secondary_strength, 0.0, 0.9))
    var cover_edge = Color(0.0, 1.0, 0.76, clamp(0.45 + secondary_strength * 0.25, 0.45, 0.95))
    for gy in range(grid_h):
        for gx in range(grid_w):
            if _lane_cell(Vector2i(gx, gy)):
                var cell_rect = Rect2i(int(floor(gx * cell_size.x + lane_expand_px)), int(floor(gy * cell_size.y + lane_expand_px)), max(1, int(ceil(cell_size.x - lane_expand_px * 2.0))), max(1, int(ceil(cell_size.y - lane_expand_px * 2.0))))
                img.fill_rect(cell_rect, lane_color)
    for block in current_cover_blocks:
        if not (block is Rect2):
            continue
        var grown = block.grow(grid_expand_px)
        img.fill_rect(Rect2i(int(grown.position.x), int(grown.position.y), int(grown.size.x), int(grown.size.y)), cover_fill)
        _stroke_rect_on_image(img, Rect2i(int(grown.position.x), int(grown.position.y), int(grown.size.x), int(grown.size.y)), cover_edge, secondary_outline_px)
    img.save_png(_secondary_map_path())
    secondary_texture = ImageTexture.create_from_image(img)
    collision_preview_texture = _build_collision_preview_texture()

func _stroke_rect_on_image(img: Image, rect: Rect2i, color: Color, thickness: int):
    var max_x = img.get_width() - 1
    var max_y = img.get_height() - 1
    for t in range(thickness):
        var left = clampi(rect.position.x - t, 0, max_x)
        var top = clampi(rect.position.y - t, 0, max_y)
        var right = clampi(rect.position.x + rect.size.x - 1 + t, 0, max_x)
        var bottom = clampi(rect.position.y + rect.size.y - 1 + t, 0, max_y)
...
func _load_secondary_texture():
    var path = _secondary_map_path()
    if path == "":
        return
    if FileAccess.file_exists(path):
        var img = Image.load_from_file(path)
...
func _rebuild_organic_collision_mask():
    organic_mask_image = null
    if collision_mode != "organic" and not organic_boundary:
        collision_preview_texture = _build_collision_preview_texture()
        return
    var width = max(1, int(map_w))
    var height = max(1, int(map_h))
    organic_mask_image = Image.create(width, height, false, Image.FORMAT_RGBA8)
    organic_mask_image.fill(Color(0, 0, 0, 1))
    var carve_radius = max(3.0, min(cell_size.x, cell_size.y) * 0.5 + lane_expand_px + organic_rounding_px)
    for gy in range(grid_h):
        for gx in range(grid_w):
            if not _lane_cell(Vector2i(gx, gy)):
                continue
            _stamp_circle_on_image(organic_mask_image, _cell_center(Vector2i(gx, gy)), carve_radius, Color(0, 0, 0, 0))
    collision_preview_texture = _build_collision_preview_texture()

func _stamp_circle_on_image(img: Image, center: Vector2, radius: float, color: Color):
    var min_x = clampi(int(floor(center.x - radius)), 0, img.get_width() - 1)
    var max_x = clampi(int(ceil(center.x + radius)), 0, img.get_width() - 1)
    var min_y = clampi(int(floor(center.y - radius)), 0, img.get_height() - 1)
    var max_y = clampi(int(ceil(center.y + radius)), 0, img.get_height() - 1)
...
func _build_collision_preview_texture() -> Texture2D:
    var width = max(1, int(map_w))
    var height = max(1, int(map_h))
    var img = Image.create(width, height, false, Image.FORMAT_RGBA8)
    img.fill(Color(0, 0, 0, 0))
    if collision_mode == "organic" or organic_boundary:
        if organic_mask_image == null:
            return null
        for y in range(height):
            for x in range(width):
                var blocked = organic_mask_image.get_pixel(x, y).a > 0.5
                img.set_pixel(x, y, Color(0.0, 0.95, 0.46, 0.18 if blocked else 0.0))
        return ImageTexture.create_from_image(img)
    for gy in range(grid_h):
        for gx in range(grid_w):
            if _point_blocked(_cell_center(Vector2i(gx, gy))):
                img.fill_rect(Rect2i(int(floor(gx * cell_size.x)), int(floor(gy * cell_size.y)), max(1, int(ceil(cell_size.x))), max(1, int(ceil(cell_size.y)))), Color(0.0, 0.95, 0.46, 0.18))
    return ImageTexture.create_from_image(img)
...
func _tick_asteroids(delta):
    ...
    player["pos"] = player_motion["pos"]
    player["vel"] = player_motion["vel"]
    ...
    if _action() and player["cool"] <= 0:
        ...
        player["cool"] = 0.18
    for r in rocks:
        var rock_motion = _move_asteroids_actor(r["pos"], r["vel"], delta, false)
        r["pos"] = rock_motion["pos"]
        r["vel"] = rock_motion["vel"]
        r["rot"] += r["spin"] * delta
        if r["pos"].distance_to(player["pos"]) < r["r"] + 15.0 * ship_scale:
            _lose_life()
...
func _tick_smash(delta):
    ...
    player["cool"] = max(0.0, player["cool"] - delta)
    var shot = _shoot_vec()
    if shot != Vector2.ZERO and player["cool"] <= 0:
        bullets.append({"pos": player["pos"], "vel": shot.normalized() * 520, "ttl": 0.9, "kind": "hero"})
        player["cool"] = 0.12
    for e in enemies:
        if e["kind"] == "spawner":
            e["timer"] -= delta
...
func _draw_grid():
    if not show_debug_grid:
        return
    var step_x = max(1, int(round(cell_size.x * 2.0)))
    var step_y = max(1, int(round(cell_size.y * 2.0)))
    for x in range(0, int(map_w), step_x):
        draw_line(Vector2(x, 0), Vector2(x, map_h), Color(1, 1, 1, 0.035), 1)
    for y in range(0, int(map_h), step_y):
...
func _draw_background_layer():
    ...
    if background_view == "photo":
        ...
        if photo_texture:
            draw_texture_rect(photo_texture, Rect2(0, 0, map_w, map_h), false, Color(1, 1, 1, secondary_photo_mix))
...
func _safe_pos(pos: Vector2) -> Vector2:
    return _cell_center(_find_nearest_walkable_cell(_pos_to_cell(pos)))

func _move_asteroids_actor(pos: Vector2, vel: Vector2, delta: float, is_player: bool) -> Dictionary:
    var damping = 0.72 if is_player else 0.9
    var bounce_nudge = max(2.0, cell_px * 0.14)
    var new_pos = pos
    var new_vel = vel
    var move_x = Vector2(new_vel.x * delta, 0.0)
    var move_y = Vector2(0.0, new_vel.y * delta)
    if move_x.length_squared() > 0.0:
        var try_x = _wrap(new_pos + move_x)
        if _point_blocked(try_x):
            new_vel.x = -new_vel.x * damping
            var retreat_x = _wrap(new_pos + Vector2(sign(new_vel.x) * bounce_nudge, 0.0))
            if not _point_blocked(retreat_x):
                new_pos = retreat_x
        else:
            new_pos = try_x
    if move_y.length_squared() > 0.0:
        var try_y = _wrap(new_pos + move_y)
        if _point_blocked(try_y):
            new_vel.y = -new_vel.y * damping
            var retreat_y = _wrap(new_pos + Vector2(0.0, sign(new_vel.y) * bounce_nudge))
            if not _point_blocked(retreat_y):
                new_pos = retreat_y
        else:
            new_pos = try_y
    if _point_blocked(new_pos):
        new_pos = _safe_pos(pos)
        new_vel *= 0.35
...
func _cell_walkable(c: Vector2i) -> bool:
    ...
    if c.x < 0 or c.x >= grid[c.y].size():
        return false
    return not _point_blocked(_cell_center(c))

func _point_blocked(pos: Vector2) -> bool:
    if collision_mode == "organic" or organic_boundary:
        if organic_mask_image == null:
            return false
        var px = clampi(int(floor(pos.x)), 0, organic_mask_image.get_width() - 1)
        var py = clampi(int(floor(pos.y)), 0, organic_mask_image.get_height() - 1)
        return organic_mask_image.get_pixel(px, py).a > 0.5
    var c = Vector2i(int(pos.x / max(0.001, cell_size.x)), int(pos.y / max(0.001, cell_size.y)))
    if c.y < 0 or c.y >= grid.size():
        return true
    if c.x < 0 or c.x >= grid[c.y].size():
        return true
...
func _pos_to_cell(pos: Vector2) -> Vector2i:
    return Vector2i(int(pos.x / max(0.001, cell_size.x)), int(pos.y / max(0.001, cell_size.y)))

func _cell_center(c: Vector2i) -> Vector2:
    return Vector2((c.x + 0.5) * cell_size.x, (c.y + 0.5) * cell_size.y)
...
func _draw_ship(pos: Vector2, angle: float, color: Color):
    var nose = 24.0 * ship_scale
    var tail = 15.0 * ship_scale
    var spine = 8.0 * ship_scale
    var wing = 12.0 * ship_scale
    var exhaust = 24.0 * ship_scale
    var pts = PackedVector2Array([
        pos + Vector2(nose, 0).rotated(angle),
        pos + Vector2(-tail, -wing).rotated(angle),
        pos + Vector2(-spine, 0).rotated(angle),
        pos + Vector2(-tail, wing).rotated(angle)
    ])
    draw_colored_polygon(pts, Color(color.r, color.g, color.b, 0.14))
    for i in range(pts.size()):
        _glow_line(pts[i], pts[(i + 1) % pts.size()], color, 1.4)
    _glow_line(pos + Vector2(-spine, 0).rotated(angle), pos + Vector2(-exhaust, 0).rotated(angle), C_MAGENTA, 1.0)
    _glow_line(pos + Vector2(-tail * 0.8, -wing * 0.66).rotated(angle), pos + Vector2(nose * 0.33, -ship_scale * 2.0).rotated(angle), Color.WHITE, 0.6)
