...
func _ready():
    ...
    if screenshot_path != "":
        ...
        if img:
            ...
            print("Failed to get viewport image in headless mode (normal on dummy renderer).")
        get_tree().quit()
...
func is_valid_pos(pos: Vector2, blocks: Array, p_idx: int) -> bool:
    for b in blocks:
        ...
        # Robust polygon boundary check to prevent clipping outside the well
        if well_polygon.size() > 2:
            var center = Vector2(nx * BLOCK_SIZE + BLOCK_SIZE/2.0, ny * BLOCK_SIZE + BLOCK_SIZE/2.0)
            if center.y >= well_bounds.position.y:
                if not Geometry2D.is_point_in_polygon(center, well_polygon):
                    return false
...
func is_playable_cell(x: int, y: int) -> bool:
    ...
    if grid[x][y] == Color(0.1, 0.1, 0.1, 0.5):
        return false
    if grid[x][y] == Color(0.0, 0.0, 0.0, 0.0):
        return false
    return true
...
func check_lines(p_idx: int):
    var lines_cleared = 0
    for y in range(grid_height):
        var has_playable_cells = false
        var all_playable_occupied = true
        for x in range(grid_width):
            if is_playable_cell(x, y):
                has_playable_cells = true
                if grid[x][y] == null:
                    all_playable_occupied = false
                    break
                    
        if has_playable_cells and all_playable_occupied:
            lines_cleared += 1
            for x in range(grid_width):
                if is_playable_cell(x, y) and grid[x][y] != null:
            ...
            for drop_y in range(y, 0, -1):
                for x in range(grid_width):
                    if is_playable_cell(x, drop_y):
                        if is_playable_cell(x, drop_y - 1):
                            grid[x][drop_y] = grid[x][drop_y - 1]
                        else:
                            grid[x][drop_y] = null
            for x in range(grid_width):
                if is_playable_cell(x, 0):
                    grid[x][0] = null
