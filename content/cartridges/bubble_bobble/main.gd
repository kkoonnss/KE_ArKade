...
func _build_ui():
    ...
    shell.alignment = BoxContainer.ALIGNMENT_END
    shell.add_theme_constant_override("separation", 28)
    margin.add_child(shell)
    var cover = PanelContainer.new()
    cover.custom_minimum_size = Vector2(420, 640)
    cover.visible = true
    cover.add_theme_stylebox_override("panel", _menu_panel_style(Color(0.95, 0.78, 0.22), Color(0.03, 0.03, 0.06, 0.96)))
    shell.add_child(cover)
    ...
    if FileAccess.file_exists(splash_path2):
        ...
        if img2:
            cover_rect.texture = ImageTexture.create_from_image(img2)
    cover.add_child(cover_rect)
...
func _setup_bubble():
    _setup_platforms()
    player["pos"] = Vector2(map_w * 0.18, platforms[platforms.size() - 1].position.y - 22)
    player["vel"] = Vector2.ZERO
    player["on_ground"] = true
    for i in range(7):
        enemies.append({"pos": _safe_pos(Vector2(map_w * randf_range(0.25, 0.8), map_h * randf_range(0.25, 0.68))), "vel": Vector2(randf_range(-60, 60), 0), "trapped": false, "trap": 0.0, "bubble": -1})
...
func _tick_bubble(delta):
    ...
    for ei in range(enemies.size() - 1, -1, -1):
        var e = enemies[ei]
        if e["trapped"]:
            var bubble_index = int(e.get("bubble", -1))
            if bubble_index >= 0 and bubble_index < bubbles.size():
                e["pos"] = bubbles[bubble_index]["pos"]
            else:
                e["pos"].y -= 55 * delta
        ...
        if e["pos"].distance_to(player["pos"]) < 22:
            _lose_life()
        for bi in range(bubbles.size()):
            var bub = bubbles[bi]
            if int(bub.get("trapped", -1)) == -1 and bub["pos"].distance_to(e["pos"]) < 34:
                e["trapped"] = true
                e["bubble"] = bi
                bub["trapped"] = ei
                bub["vel"] = Vector2(0, -42)
                bub["ttl"] = 4.0
                bubbles[bi] = bub
                _burst(e["pos"], C_CYAN, 10)
                break
    if enemies.is_empty():
        wave += 1
        _setup_bubble()
