...
- [ ] **Hub post-corruption regression** — thumbnails and favorites reported missing in the live hub UI on 2026-06-30 even though `app/hub/main.gd` contains favorites array + thumbnail-loading code. An agent was dispatched to fix. Tracked as `pending_kons_verify` until a real receipt for that session lands.
