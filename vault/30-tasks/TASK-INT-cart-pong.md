...
status: ready
owner_agent: ""
touches: [content/cartridges/pong]
locks_required: [cart-pong]
...
## Objective
Bring **pong** onto the shared WELL/FILL adapter (solid region + container boundary -> a fillable shape) so it builds its level from the painted map, and give it the shared secondary-controls Tab menu. Archetype: **well_fill** — rollout (Wave 2).

## Knobs to expose
invert, fill, grid_scale, density, bounds_clamp, wall_width (expose only the ones that meaningfully change this game).

## Notes
- Own ONLY `content/cartridges/pong/`. Read `app/shared/**` + the map read-only. Lock: `cart-pong`.
