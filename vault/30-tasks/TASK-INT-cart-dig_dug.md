...
## Objective
Bring **dig_dug** onto the shared MAZE adapter (grid walkable cells -> node graph / corridors) so it builds its level from the painted map, and give it the shared secondary-controls Tab menu. Archetype: **maze** — rollout (Wave 2).
...
## Notes
- Own ONLY `content/cartridges/dig_dug/`. Read `app/shared/**` + the map read-only. Lock: `cart-dig_dug`.
