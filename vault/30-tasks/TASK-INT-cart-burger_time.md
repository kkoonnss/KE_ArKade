---
task_id: TASK-INT-cart-burger_time
stage: 6
wave: 2
priority: P2
lane: cartridge
...
locks_required: [cart-burger_time]
...
## Objective
Bring **burger_time** onto the shared PLATFORM adapter (platform_top edges + procedurally added platforms; gravity) so it builds its level from the painted map, and give it the shared secondary-controls Tab menu. Archetype: **platform** — rollout (Wave 2).
...
## Notes
- Own ONLY `content/cartridges/burger_time/`. Read `app/shared/**` + the map read-only. Lock: `cart-burger_time`.
