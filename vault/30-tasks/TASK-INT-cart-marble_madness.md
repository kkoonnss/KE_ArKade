---
task_id: TASK-INT-cart-marble_madness
stage: 6
wave: 2
priority: P2
lane: cartridge
...
locks_required: [cart-marble_madness]
...
## Objective
Bring **marble_madness** onto the shared MAZE adapter (grid walkable cells -> node graph / corridors) so it builds its level from the painted map, and give it the shared secondary-controls Tab menu. Archetype: **maze** — rollout (Wave 2).
...
## Notes
- Own ONLY `content/cartridges/marble_madness/`. Read `app/shared/**` + the map read-only. Lock: `cart-marble_madness`.
