...
status: ready
owner_agent: ""
touches: [content/cartridges/tron]
locks_required: [cart-tron]
...
## Objective
Bring **tron** onto the shared MAZE adapter (grid walkable cells -> node graph / corridors) so it builds its level from the painted map, and give it the shared secondary-controls Tab menu. Archetype: **maze** — rollout (Wave 2).
...
## Notes
- Own ONLY `content/cartridges/tron/`. Read `app/shared/**` + the map read-only. Lock: `cart-tron`.
