---
task_id: TASK-INT-cart-bomberman
stage: 6
wave: 2
priority: P2
lane: cartridge
...
status: ready
owner_agent: ""
touches: [content/cartridges/bomberman]
locks_required: [cart-bomberman]
...
## Objective
Bring **bomberman** onto the shared WELL/FILL adapter (solid region + container boundary -> a fillable shape) so it builds its level from the painted map, and give it the shared secondary-controls Tab menu. Archetype: **well_fill** — rollout (Wave 2).

## Knobs to expose
invert, fill, grid_scale, density, bounds_clamp, wall_width (expose only the ones that meaningfully change this game).

## Notes
- Own ONLY `content/cartridges/bomberman/`. Read `app/shared/**` + the map read-only. Lock: `cart-bomberman`.
