...
- `headless_launch`: FAIL (ENVIRONMENT-BLOCKED)
  ...
  output:
```text
ERROR: Failed to open user://logs/godot2026-06-25T12.39.35.log
...
======================================================
```
- `level_smoke`: FAIL (ENVIRONMENT-BLOCKED)
  ...
  output:
```text
ERROR: Failed to open user://logs/godot2026-06-25T12.39.40.log
   at: (core/io/dir_access.cpp:351)
...
======================================================
```
- `screenshot_smoke`: TIMEOUT (ENVIRONMENT-BLOCKED)
  ...
  output:
```text
ERROR: Failed to open user://logs/godot2026-06-25T12.39.45.log
   at: (core/io/dir_access.cpp:351)
...
- This run was partially blocked by the environment before Godot could fully initialize, so the result is not a clean cartridge verdict yet.
- This run was partially blocked by the environment before Godot could fully initialize, so the result is not a clean cartridge verdict yet.
- This run was partially blocked by the environment before Godot could fully initialize, so the result is not a clean cartridge verdict yet.
